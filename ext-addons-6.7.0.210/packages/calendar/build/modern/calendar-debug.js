...
Ext.define('Ext.calendar.date.Util', {
    ...
    privates: {
        ...
        /**
         * Expand a date range via UTC, set the start to 0 hours, set the end
         * to 0 hours (if needed).
...
Ext.define('Ext.calendar.dd.WeeksTarget', {
    ...
    onDrop: function(info) {
        ...
        view.handleChange('drop', event, new Ext.calendar.date.Range(// here we pass false on preventDstAdsjust because we want the time to remain the same
        ...
        D.add(event.getStartDate(), D.DAY, difference, false), D.add(event.getEndDate(), D.DAY, difference, false)), function() {
            view.clearSelected();
        });
        this.callParent([
            info
...
Ext.define('Ext.calendar.form.CalendarPicker', {
    ...
    floatedPicker: {
        ...
        itemTpl: '<div class="' + Ext.baseCSSPrefix + 'calendar-picker-list-icon" style="background-color: {color};"></div>' + '<span class="' + Ext.baseCSSPrefix + 'calendar-picker-list-text ' + Ext.baseCSSPrefix + 'list-label">{title:htmlEncode}</span>'
    },
    ...
    createEdgePicker: function() {
        ...
        /* eslint-disable max-len, indent */
        picker.slots[0].itemTpl = '<div class="' + Ext.baseCSSPrefix + 'picker-item {cls}">' + '<div class="' + Ext.baseCSSPrefix + 'calendar-picker-list-icon" style="background-color: {color};"></div>' + '<span class="' + Ext.baseCSSPrefix + 'list-label">{title:htmlEncode}</span>' + '</div>';
        /* eslint-enable max-len, indent */
        return picker;
    },
    prepareData: function(data, index, record) {
...
Ext.define('Ext.calendar.store.Events', {
    ...
    setRange: function(range) {
        var me = this,
            ...
            fetchCount = 0,
            isLeading = false,
            fetchStart, fetchEnd, fullStart, fullEnd, newRange;
...
Ext.define('Ext.calendar.panel.Base', {
    ...
    privates: {
        /**
         * @method calculateMoveNext
         * @inheritdoc Ext.calendar.view.Base#method-calculateMoveNext
         * @private
         */
        ...
        /**
         * @method calculateMovePrevious
         * @inheritdoc Ext.calendar.view.Base#method-calculateMovePrevious
         * @private
         */
...
Ext.define('Ext.calendar.view.Base', {
    ...
    privates: {
        ...
        toUtcOffset: function(date) {
            ...
            return D.add(d, D.MINUTE, tzOffset, false);
        },
        /**
...
Ext.define('Ext.calendar.view.Days', {
    ...
    privates: {
         ...
         * @param {Boolean} [clearAll=false] `true` to clear all nodes. `false` to leave
         * the last node (gutter).
         *
         * @private
         */
        ...
        createScroller: function() {
            return new Ext.scroll.NativeScroller({
        ...
        doRefresh: function() {
            ...
            timeContainer.appendChild(me.generateTimeElements(), true);
        ...
        onBodyTouchMove: function(e) {
            var me = this,
                ...
                el, w, slot, startSlot, topSlot, bottomSlot, start, end;
            if (!creating) {
                return;
            }
            w = creating.widget;
 ...
 * using the {@link Ext.calendar.view.Days#visibleDays} config option. By default, the calendar
 ...
 * {@link Ext.calendar.view.Days#cfg-endTime} configs.  The time displayed on the timeline may be
 * customized using either the {@link Ext.calendar.view.Days#timeFormat} or
...
Ext.define('Ext.calendar.view.WeeksRenderer', {
    ...
    constructor: function(config) {
        ...
        Ext.apply(me, config);
        start = me.start;
        me.end = D.add(start, D.DAY, me.days);
    ...
    calculate: function() {
        ...
        for (i = 0; i < days; ++i) {
                    ...
                    if (!seen[id]) {
                        ...
                        if (isSpan) {
                            ...
                            if (eventEnd > offsetRangeEnd) {
                                // If the event finishes after our range, then just span it
                                // to the end
                                span = days - i;
                            } else {
            ...
            if (hasMaxEvents) {
                ...
                if (eLen > 0) {
                    if (eLen > maxEvents) {
                        // This is a naive attempt to calculate overflows, without taking
                        // into account other days in the row. We'll use this as the basis
                        // going forward if we need to do any extra calculations once we start
                        // considering other days.
                        // -1 here because maxEvents is the total we can show, without the
                        // "show more" item.
                        // Assuming that "show more" is roughly the same size as an event,
                        // which we'll also need to show, we have to lop off another event.
...
Ext.define('Ext.calendar.view.Weeks', {
    ...
    privates: {
         ...
         * @return {Number} return.heightForEvents The available height for displaying events
         * in a cell.
         *
         * @private
         */
...
Ext.define('Ext.calendar.view.Month', {
    ...
    config: {
         ...
         * The number of weeks to show in this view. If specified as `null`, the view will generate
         * the appropriate number of rows to display a full month based on the passed
         * {@link #cfg!value}. In a majority of cases, this will be 5, however some months
         * will only require 4, while others will need 6. Defaults to the largest value to keep
         * the view size consistent.
         */
        visibleWeeks: 6
    },
    /**
...
Ext.define('Ext.calendar.panel.Panel', {
    ...
    config: {
         ...
         * for each item. Use a number less than 0 to insert at the front. Use a number
         * larger than 100 to insert at the end.
         */
        sideBar: {
            xtype: 'panel',
